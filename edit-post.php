        ...
        <div class="input-group mb-3">
          <select class="form-select" name="category_id" aria-label="Default select example">
            ...
            while ($data = mysqli_fetch_array($execute_get_categories_query)) {
              ...
              $cat_name = $data['name'];
              ?>
              <option value=<?= $cat_id; ?>   <?= ($cat_id == $row_category_id) ? "selected" : ""; ?>><?= $cat_name ?></option>
              <?php
            }
            ?>
          </select>
...
if (isset($_POST['submit'])) {
  ...
  if (empty($title) || empty($description) || empty($category_id)) {
    ...
    $update_blog_post_query = "UPDATE posts SET title='$title', description='$description', category_id='$category_id', is_draft='$is_draft' WHERE id='$update_id'";
    $execute_update_blog_post_query = mysqli_query($conn, $update_blog_post_query);

    if ($execute_update_blog_post_query) {
      echo '<script>toastr.success("Post updated successfully")</script>';
      redirect_to(2, "my-posts.php");
      // header("Location:my-posts.php");
    } else {
      $error = mysqli_error($conn);
      echo "Error: $error";
      echo '<script>toastr.error("Something went wrong while editing blog post")</script>';
    }
